...
multitask :push do
  ...
  if strippedStatus == ""
    ...
    abort "## ERROR: Couldn't checkout master" unless system "git checkout master"
    abort "## ERROR: Couldn't reset master" unless system "git reset --hard origin/master"
    abort "## ERROR: Couldn't checkout source" unless system "git checkout source"
...
task :s3 do
  ...
  ok_failed system("s3cmd --config=#{s3cmd_config} sync --acl-public --reduced-redundancy public/* s3://#{s3_bucket}/")
  ok_failed system("s3cmd --config=#{s3cmd_config} put --acl-public --reduced-redundancy public/feed/atom.xml s3://#{s3_bucket}/feed/index.html")
end
